...
def forward_step(
    ...
    return first_term - second_term + third_term #according to eq.2
...
def reverse_step(
    ...
    return first_term + second_term - third_term  #according to eq.3
...
def coupled_stablediffusion(prompt="",
    ...
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):#key part of EDICT
        t_scale = t / schedulers[0].num_train_timesteps

        if (reverse) and (not run_baseline):#according to eq.13
        ...
        for latent_i in range(2): #latent_i=0 for baseline; latent_i=0,1 for EDICT
            ...
            noise_pred = noise_pred_uncond + guidance_scale * grad #add pseudo-gradient
